...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/oa/hrmRegularApply/form?id=${hrmRegularApply.id}">转正申请<shiro:hasPermission name="oa:hrmRegularApply:edit">${not empty hrmRegularApply.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="oa:hrmRegularApply:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="hrmRegularApply" action="${ctx}/oa/hrmRegularApply/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<sys:treeselect id="hrmUser" name="hrmUser.id" value="${hrmRegularApply.hrmUser.id}" labelName="" labelValue="${hrmRegularApply.hrmUser.name}"
		...
		<div class="form-actions">
			...
			<shiro:hasPermission name="oa:hrmRegularApply:edit">
				...
				<c:if test="${hrmRegularApply.act.taskDefKey ne 'apply_end'}">
					...
					<input id="btnSubmit" class="btn btn-inverse" type="submit" value="驳回到主管" onclick="$('#flag').val('3')"/>&nbsp;
				</c:if>
			</shiro:hasPermission>
